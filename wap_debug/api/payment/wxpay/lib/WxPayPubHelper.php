...
class Wxpay_client_pub extends Common_util_pub 
{
	...
	{
		//error_log('data:'.date('Y-m-d H:i:s',time())."\n".var_export($parameter,1)."\n",3,'D:/WWW/jph/huadian/log5.txt');
		//error_log('data:'.date('Y-m-d H:i:s',time())."\n".var_export($parameterValue,1)."\n",3,'D:/WWW/jph/huadian/log5.txt');
		$this->parameters[$this->trimString($parameter)] = $this->trimString($parameterValue);
		//error_log('data:'.date('Y-m-d H:i:s',time())."\n".var_export($this->trimString($parameterValue),1)."\n",3,'D:/WWW/jph/huadian/log8.txt');
	}
	...
	function postXml()
	{
		...
		//error_log('data:'.date('Y-m-d H:i:s',time())."\n".var_export($this->response,1)."\n",3,'D:/WWW/jph/huadian/log3.txt');
...
class JsApi_pub extends Common_util_pub
{
	...
	function getButton($code,$returnrul) {
			$button = <<<EOT
		...
		function jsApiCall(){
			WeixinJSBridge.invoke('getBrandWCPayRequest',{$code},function(res){
					...
					if(res.err_msg.indexOf('ok')>0){
						//alert('成功路经:'+$returnrul);
						window.location.href='http://test.luckyorchid.net/jph/huadian/mobile/user.php?act=order_list&sn=$returnrul';
						
					}else{
						window.location.href='http://test.luckyorchid.net/jph/huadian/mobile/user.php?act=order_list';
					}
				});
		}
