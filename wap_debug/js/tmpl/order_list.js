...
//处理商品状态
    $('.keyorder').click(function(){
        var order_state = parseInt($("input[name=order_state]").val());
        var curkey = $(this).attr('order_state');//10.待付款 20.待发货 3.待收货 4.待评价
        
        $(this).addClass("current").siblings().removeClass("current");        
        var url = ApiUrl+"/index.php?act=member_order&op=order_list&page="+page+"&curpage="+curpage+"&getpayment=true&order_state="+curkey;
        $.ajax({
            url:url,
            type:'post',
            data:{key:key},
