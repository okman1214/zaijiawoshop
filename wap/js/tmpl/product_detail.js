...
var store_id;
    ...
    var str='&from=';
    var str_before = share_url.split(str)[0];   
    var str_after = share_url.split(str)[1];  
    if(str_after!=null){
      location.href=str_before;
    }
$(function () {
        ...
        $.ajax({
            ...
            success: function (result) {
                ...
                if (!data.error) {
                    ...
                    $('#wx_title').html(data.goods_info.goods_name);
                    $('#wx_desc').html(data.goods_info.goods_jingle);
                    $('#wx_img').html(data.goods_image[0]);
                    ...
                    //渲染模板
                    var html = template.render('alert-content-cnt', data);
                    $(".alert-content").html(html);
                     var href_url=window.location.href;
                      $(".alert-qrcode-i").html('');
                      jQuery(".alert-qrcode-i").qrcode({
                          typeNumber: 0,      //计算模式
                          correctLevel: 0,//纠错等级
                          text:href_url
                      });
                    ...
                    var _ = template.render('product_detail_sepc', data);
                    $('#product_detail_spec_html').html(_);
                    var _ = template.render('voucher_script', data);
                    $('#voucher_html').html(_);
                    ...
                    if (data.video_path) {
                        var player = new prismplayer({
                            ...
                            source: data.video_path, // 视频url 支持互联网可直接访问的视频地址
                            autoplay: true, // 自动播放
                            width: "100%", // 播放器宽度
                            height: "16rem", // 播放器高度
                            skinLayout: false
...
function loadwechat(){
    $.ajax({
        url: ApiUrl + "/index.php?act=index&op=loadwechat",
        type: 'get',
        data: {
                share_url:share_url
            },
        dataType: 'json',
        success: function(result) {
            $('#appId').html(result.datas.signPackage.appId);
            $('#timestamp').html(result.datas.signPackage.timestamp);
            $('#nonceStr').html(result.datas.signPackage.nonceStr);
            $('#signature').html(result.datas.signPackage.signature);
      wx.config({
      debug: false,
      appId:$('#appId').html(),
   ...
   wx.ready(function () {
    // 在这里调用 API
			var wx_title=$('#wx_title').html();
			var wx_desc= $('#wx_desc').html();
      var wx_link=location.href;
			var wx_img=$('#wx_img').html(); 
