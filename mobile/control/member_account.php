...
class member_accountControl extends mobileMemberControl {
    ...
    public function bind_mobile_step1Op() {
        ...
        try {
            ...
            $coder_id=$tpl_info['coder_id'];
			$sms = new Sms();
			$message_tml=array('content'=>$message);
       
		
			  $result = $sms->send($message_tml,$coder_id,$_POST["mobile"]);
    ...
    public function get_mobile_infoOp() {
        ...
        $data['member_mobile'] = $data['state'] ? encryptShow($this->member_info['member_mobile'],4,4) : $this->member_info['member_mobile'];
    ...
    private function _send_bind_mobile_msg() {
        ...
        try {
			...
			$result = $sms->send($message_tml,$coder_id,$this->member_info['member_mobile']);
            if ($result) {
                $update_data = array();
                $update_data['auth_code'] = $verify_code;
                $update_data['send_acode_time'] = TIMESTAMP;
