...
class indexControl extends mobileHomeControl{
    ...
    }		public function loadwechatOp(){		require_once(BASE_DATA_PATH.DS.'api'.DS.'wxlogin'.DS.'comm'.DS."config.php");		require_once(BASE_DATA_PATH.DS.'api'.DS.'wxlogin'.DS.'share'.DS."jssdk.php");		$jssdk = new JSSDK($_SESSION["appid"],$_SESSION["appkey"]);		$signPackage = $jssdk->getSignPackage($_GET['share_url']);		output_data(array('signPackage'=>$signPackage,'member_id'=>base64_encode($this->member_info['member_id'])));	}
	/**
     * 高级搜索
     */
